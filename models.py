...
class Member(db.Model):
    ...
    transactions = db.relationship(
        "Transaction", backref="member", cascade="all, delete"
    )
    ...
    def get_outstanding_charges(self):
        ...
        pending_transactions = Transaction.query.filter_by(
            member_id=self.id, is_returned=False
        ).all()
...
class Transaction(db.Model):
    ...
    def calculate_charges(self):
        ...
        # Normal rent charges
        charges += (
            min(days, app.config["BOOK_RETURN_DEADLINE"])
            * app.config["PER_DAY_RENT"]
            * self.count
        )
        # Fine charges
        charges += (
            max(days - app.config["BOOK_RETURN_DEADLINE"], 0)
